...
namespace stoppingcosmicmuonselection {
  ...
  public:
    ...
    std::vector<std::vector<double>> GetRightCalo(art::Event& evt, double &T0, const recob::Track &track);
  ...
  std::vector<std::vector<double>> GetRightCalo(art::Event& evt, double &T0, const recob::Track &track)
  {
    ...
    double drift_velocity = detprop.DriftVelocity()*1e-3;
        ...
        for (size_t itp = 0; itp < tracklist[trkIter]->NumberTrajectoryPoints(); ++itp){
          ...
          if (tpcid.isValid) {
            try{
              ...
              if(sce->EnableCalSpatialSCE()&&fSCE) posOffsets = sce->GetCalPosOffsets(pos,tpcid.TPC);
  ...
  void FixCalo::GetPitch(art::Ptr<recob::Hit> hit, std::vector<double> trkx, std::vector<double> trky, std::vector<double> trkz, std::vector<double> trkw, std::vector<double> trkx0, double *xyz3d, double &pitch, double TickT0){
